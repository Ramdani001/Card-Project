...
const Card = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const url = `/api/cards?page=1&limit=${rowsPerPage}/`;
  ...
  const handleEdit = (idCard: number) => {
    alert(idCard);
  };

  const handleDelete = async (idCard: number) => {
    try {
      await axios.delete(`http://localhost:3000/api/cards/${idCard}`);
      // Hapus card dari state supaya UI update
      setElements((prev: any) => prev.filter((c: any) => c.idCard !== idCard));
  ...
  const rows = currentRows.map((item: any) => (
    <Table.Tr key={item.idCard}>
      {/* Image */}
      <Table.Td>
        <Image src={item.detail.image.location} alt={item.detail.name} width={60} />
      </Table.Td>
      ...
      {/* Price */}
      <Table.Td>Rp {item.detail.price.toLocaleString("id-ID")}</Table.Td>
    ...
    <>
      <Container style={{ minWidth: "80vw" }}>
        ...
        <Modal opened={opened} onClose={close} title="Add Card">
          <form>
            <Stack>
              <TextInput label="Name" placeholder="Yu-Gi oh" radius="md" />
              ...
              <TextInput label="Price" placeholder="Yu-Gi oh" radius="md" />
              <TextInput label="Stok" placeholder="Yu-Gi oh" radius="md" />
            ...
            <Flex pt="md" gap="xl" align="center">
