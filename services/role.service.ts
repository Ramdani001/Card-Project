...
export const getRoles = async (options: Prisma.RoleFindManyArgs) => {
  const finalOptions: Prisma.RoleFindManyArgs = {
    ...
    include: {
      ...
      cardCategoryRoleAccesses: { select: { category: true } },
...
export const createRole = async ({ name, categoryIds = [], menuIds = [] }: CreateRoleParams) => {
  ...
  return await prisma.role.create({
    ...
    include: {
      cardCategoryRoleAccesses: true,
    },
  });
};

export const updateRole = async ({ id, name, categoryIds = [], menuIds = [] }: UpdateRoleParams) => {
  const existingRole = await prisma.role.findUnique({
    where: { id },
