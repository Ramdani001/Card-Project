...
const ListVillage = () => {
  const [villages, setProvincies] = useState<VillageDto[]>([]);
  ...
  const [selectedVillage, setSelectedVillage] = useState<VillageDto | null>(null);
  ...
  const fetchProvincies = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/villages?${buildQuery()}`);
  ...
  const handleDelete = (id: string) => {
    openConfirmModal({
      ...
      centered: true,
      children: <Text size="sm">Are you sure you want to delete this province? This action cannot be undone.</Text>,
  ...
  const handleOpenEdit = (province: VillageDto) => {
    setSelectedVillage(province);
    open();
  };

  const columns: ColumnDef<VillageDto>[] = [
    {
      key: "no",
      label: "No",
      width: 60,
  ...
  return (
    <Paper shadow="xs" p="md" radius="md" withBorder>
      ...
      <VillageForm opened={opened} onClose={close} villageToEdit={selectedVillage} onSuccess={fetchProvincies} />
    </Paper>
  );
};
