...
const ListProvince = () => {
  ...
  const fetchProvincies = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/provincies?${buildQuery()}`);
  ...
  const handleDelete = (id: string) => {
    openConfirmModal({
      ...
      onConfirm: async () => {
        try {
          const res = await fetch(`/api/provincies/${id}`, {
            method: "DELETE",
          });
          const json = await res.json();
  ...
  const handleOpenEdit = (province: ProvinceDto) => {
    setSelectedProvince(province);
    open();
  };

  const columns: ColumnDef<ProvinceDto>[] = [
    {
      key: "no",
      label: "No",
      width: 60,
  ...
  return (
    <Paper shadow="xs" p="md" radius="md" withBorder>
      ...
      <ProvinceForm opened={opened} onClose={close} provinceToEdit={selectedProvince} onSuccess={fetchProvincies} />
    </Paper>
  );
};
