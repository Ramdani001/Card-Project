...
const ListCourier = () => {
  ...
  const fetchCouriers = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/couriers?${buildQuery()}`);
  ...
  const handleDelete = (id: string) => {
    openConfirmModal({
      ...
      centered: true,
      children: <Text size="sm">Are you sure you want to delete this courier? This action cannot be undone.</Text>,
  ...
  const handleOpenEdit = (courier: CourierDto) => {
    setSelectedCourier(courier);
    open();
  };

  const columns: ColumnDef<CourierDto>[] = [
    {
      key: "no",
      label: "No",
      width: 60,
  ...
  return (
    <Paper shadow="xs" p="md" radius="md" withBorder>
      <Flex justify="space-between" align="center" mb="lg">
        <Title order={3}>Couriers</Title>

        <Group>
          <ActionIcon variant="default" size="lg" onClick={fetchCouriers} loading={loading}>
            <IconRefresh size={18} />
          </ActionIcon>
      ...
      <CourierForm opened={opened} onClose={close} courierToEdit={selectedCourier} onSuccess={fetchCouriers} />
    </Paper>
  );
};
