...
const ListSubDistrict = () => {
  const [cities, setSubDistricts] = useState<SubDistrictDto[]>([]);
  ...
  const [selectedSubDistrict, setSelectedSubDistrict] = useState<SubDistrictDto | null>(null);
  ...
  const fetchSubDistricts = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/sub-districts?${buildQuery()}`);
  ...
  const handleDelete = (id: string) => {
    openConfirmModal({
      ...
      centered: true,
      children: <Text size="sm">Are you sure you want to delete this sub district? This action cannot be undone.</Text>,
      labels: { confirm: "Delete", cancel: "Cancel" },
      confirmProps: { color: "red" },
      onConfirm: async () => {
        try {
          const res = await fetch(`/api/sub-districts/${id}`, {
            method: "DELETE",
          });
          const json = await res.json();
  ...
  const handleOpenEdit = (subDistrict: SubDistrictDto) => {
    setSelectedSubDistrict(subDistrict);
    open();
  };

  const columns: ColumnDef<SubDistrictDto>[] = [
    {
      key: "no",
      label: "No",
      width: 60,
  ...
  return (
    <Paper shadow="xs" p="md" radius="md" withBorder>
      <Flex justify="space-between" align="center" mb="lg">
        <Title order={3}>Sub Districts</Title>

        <Group>
          <ActionIcon variant="default" size="lg" onClick={fetchSubDistricts} loading={loading}>
            <IconRefresh size={18} />
          </ActionIcon>
      ...
      <SubDistrictForm opened={opened} onClose={close} subDistrictToEdit={selectedSubDistrict} onSuccess={fetchSubDistricts} />
    </Paper>
  );
};

export default ListSubDistrict;
