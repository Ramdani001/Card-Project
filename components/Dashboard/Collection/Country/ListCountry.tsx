...
const ListCountry = () => {
  ...
  const fetchCountries = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/countries?${buildQuery()}`);
  ...
  const handleDelete = (id: string) => {
    openConfirmModal({
      ...
      centered: true,
      children: <Text size="sm">Are you sure you want to delete this country? This action cannot be undone.</Text>,
  ...
  return (
    <Paper shadow="xs" p="md" radius="md" withBorder>
      <Flex justify="space-between" align="center" mb="lg">
        ...
        <Group>
          <ActionIcon variant="default" size="lg" onClick={fetchCountries} loading={loading}>
            <IconRefresh size={18} />
          </ActionIcon>
      ...
      <CountryForm opened={opened} onClose={close} countryToEdit={selectedCountry} onSuccess={fetchCountries} />
    </Paper>
  );
};
